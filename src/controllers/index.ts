...
export function getProxies(req: Request, res: Response) {
	try {
		...
		Proxy.count(goodOnly)
			.then((n) => Math.ceil(n / proxiesPerPage))
			.then((totalPages) => {
				if (page >= totalPages) {
					res.status(0 === totalPages ? 200 : 404).json({ proxies: [], page, totalPages })
	...
	try {
		req.body.schemes.forEach((scheme: string) => {
			for (let port = req.body.ports[0]; port <= req.body.ports[1]; ++port) {
				for (
					let address = ip2number(req.body.addresses[0]);
					address <= ip2number(req.body.addresses[1]);
					++address
				) {
					new Proxy(scheme, number2ip(address), port).insert().catch(() => {
						/*
